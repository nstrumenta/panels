...
} from '@base/services/ILayoutManager';
import {
  ILayoutStorage,
  ISO8601Timestamp,
  Layout,
...
} from '@base/services/ILayoutStorage';
import { IRemoteLayoutStorage, RemoteLayout } from '@base/services/IRemoteLayoutStorage';
...
export default class LayoutManager implements ILayoutManager {
  ...
  public async deleteLayout({ id }: { id: LayoutID }): Promise<void> {
    ...
    if (this.remote && !layoutIsShared(localLayout)) {
      const layout: Layout = localLayout;
      await this.local.put({
        ...
        syncInfo: {
          ...
          lastRemoteSavedAt: layout.syncInfo?.lastRemoteSavedAt,
        },
      });
    } else {
  ...
  public async overwriteLayout({ id }: { id: LayoutID }): Promise<Layout> {
    ...
    if (layoutIsShared(localLayout)) {
      ...
      const result = await this.local.put({
        ...
        working: undefined,
        syncInfo:
          this.remote && layout.syncInfo?.status !== 'new'
            ? { status: 'updated', lastRemoteSavedAt: layout.syncInfo?.lastRemoteSavedAt }
            : layout.syncInfo,
      });
