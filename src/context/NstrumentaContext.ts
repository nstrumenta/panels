...
export function useCurrentUser(): CurrentUser {
  ...
  if (nstrumentaContext) {
    ...
    return {
      currentUser: firebaseInstance?.user,
      signIn: () => {
        firebaseInstance?.auth && signInWithPopup(firebaseInstance.auth, new GithubAuthProvider());
      },
      signOut: async () => {
        firebaseInstance?.auth && firebaseInstance?.auth.signOut();
      },
    };
  } else {
    return { currentUser: undefined, signIn: () => {} };
  }
}
