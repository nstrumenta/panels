...
function useFeatures(): Feature[] {
  const features: Feature[] = [
    ...
    },
    {
      ...
      description: (
        <>
          {'newNavigationDescription'}
          {isDesktopApp() && 'restartTheAppForChangesToTakeEffect'}
        </>
      ),
    },
    {
      ...
      name: 'newImagePanel',
      description: <>{'newImagePanelDescription'}</>,
    },
  ];
  ...
  if (isDesktopApp()) {
    features.push({
      ...
      name: 'ros2NativeConnection',
      description: (
        <>
          {'ros2NativeConnectionDescription'} {'restartTheAppForChangesToTakeEffect'}
        </>
      ),
    });
  }
...
export const ExperimentalFeatureSettings = (): React.ReactElement => {
  ...
  return (
    <Stack gap={2}>
      {features.length === 0 && (
        <Typography fontStyle="italic">{'noExperimentalFeatures'}</Typography>
      )}
